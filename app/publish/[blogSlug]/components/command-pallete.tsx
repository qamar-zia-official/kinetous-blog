...
const TEXT_COLORS: GeneratedColor[] = Object.entries(TAILWIND_HUES).map(
    ([name, base]) => ({
        name,
...
const HIGHLIGHT_COLORS: GeneratedColor[] = Object.entries(TAILWIND_HUES).map(
    ([name, base]) => ({
        name,
        label: name[0].toUpperCase() + name.slice(1),
...
function ColorSwatch({ hex, ring }: { hex: string; ring?: boolean }) {
    return (
        <span
            ...
            style={{
                backgroundColor: hex,
                boxShadow: ring ? `0 0 0 2px ${hex}33` : undefined,
            }}
            aria-hidden
        />
    );
...
export const turnIntoActions: EditorAction[] = [
    ...
    },
    {
        id: "bullet-list",
        label: "Bullet List",
        keywords: ["ul", "list", "bullet", "points"],
        icon: List,
...
export const formatActions: EditorAction[] = [
    ...
    },
    {
        ...
        keywords: ["clear", "reset", "plain", "eraser"],
        shortcut: "Ctrl+\\",
        icon: Eraser,
...
export const alignActions: EditorAction[] = [
    {
        ...
        icon: AlignLeft,
        action: (e) => (e.chain().focus() as any).setTextAlign("left").run(),
    },
    {
        id: "align-center",
        label: "Align Center",
        ...
        icon: AlignCenter,
        action: (e) => (e.chain().focus() as any).setTextAlign("center").run(),
    },
    {
        id: "align-right",
        label: "Align Right",
        ...
        icon: AlignRight,
        action: (e) => (e.chain().focus() as any).setTextAlign("right").run(),
    },
    {
        id: "align-justify",
        label: "Justify",
        ...
        icon: AlignJustify,
        action: (e) => (e.chain().focus() as any).setTextAlign("justify").run(),
    },
    {
        id: "indent",
    ...
    },
    {
        ...
        keywords: ["outdent", "unnest", "shift tab"],
        icon: IndentDecrease,
...
export const colorActions: EditorAction[] = [
    {
        ...
        keywords: ["color", "reset", "clear"],
        icon: Baseline,
        action: (e) => (e.chain().focus() as any).unsetColor().run(),
    },
...
export const highlightActions: EditorAction[] = [
    {
        ...
        keywords: ["highlight", "reset", "clear"],
        icon: Ban,
    ...
    ...HIGHLIGHT_COLORS.map<EditorAction>((c) => ({
        ...
        swatch: c.hex,
        action: (e) =>
            e.chain().focus().toggleHighlight({ color: c.hex }).run(),
    })),
];
...
export const insertionActions: EditorAction[] = [
    {
        id: "table",
        label: "Insert Table",
        keywords: ["table", "grid", "spreadsheet", "cols", "rows"],
        icon: TableIcon,
        action: (e) =>
            e
                .chain()
...
export const tableActions: EditorAction[] = [
    ...
    },
    {
        ...
        keywords: ["table", "next", "tab"],
        icon: TextCursorInput,
        action: (e) => e.chain().focus().goToNextCell().run(),
    },
    {
        id: "prev-cell",
        label: "Go To Previous Cell",
        keywords: ["table", "previous", "shift tab"],
        icon: TextCursorInput,
        action: (e) => e.chain().focus().goToPreviousCell().run(),
    },
    {
        id: "fix-tables",
        label: "Fix Malformed Table",
        keywords: ["table", "fix", "repair"],
        icon: TableIcon,
        action: (e) => e.chain().focus().fixTables().run(),
    },
    {
        id: "delete-table",
        label: "Delete Table",
...
export const historyActions: EditorAction[] = [
    ...
    },
    {
        ...
        action: (e) => e.chain().focus().selectAll().run(),
    },
];
        ...
        run: (ctx) => {
            if (ctx.slug)
                navigator.clipboard.writeText(
                    `${window.location.origin}/blog/${ctx.slug}`,
                );
        },
    },
    {
...
type RecentEntry = {
    group: Exclude<CommandGroupKey, "emoji">;
    id: string;
    label: string;
};
...
function pushRecent(entry: RecentEntry) {
    const existing = readRecents().filter(
        (r) => !(r.group === entry.group && r.id === entry.id),
    );
...
const GROUP_LABELS: Record<
    Exclude<CommandGroupKey, "app" | "emoji">,
    string
> = {
    turnInto: "Turn Into",
    format: "Format",
...
export default function CommandPallete({
    ...
    const filtered = useMemo(() => {
        return {
            ...
            turnInto: turnIntoActions
                ...
                .slice(0, 12),
            format: formatActions.filter((a) =>
                matches(query, a.label, a.keywords),
            ),
            align: alignActions.filter((a) =>
                matches(query, a.label, a.keywords),
            ),
            ...
            color: query
                ? colorActions.filter((a) =>
                      matches(query, a.label, a.keywords),
                  )
                : colorActions.slice(0, 1),
            highlight: query
                ? highlightActions.filter((a) =>
                      matches(query, a.label, a.keywords),
                  )
            ...
            insert: insertionActions
                .filter((a) => matches(query, a.label, a.keywords))
                .slice(0, 12),
            table: tableActions.filter((a) =>
                matches(query, a.label, a.keywords),
            ),
            history: historyActions.filter((a) =>
                matches(query, a.label, a.keywords),
            ),
            emoji: query
                ? gitHubEmojis
    ...
    function runEditorAction(
        group: Exclude<CommandGroupKey, "emoji" | "app">,
        item: EditorAction,
    ) {
        item.action(editor);
    ...
    function runRecent(entry: RecentEntry) {
        const pools: Record<
            Exclude<CommandGroupKey, "emoji">,
            (EditorAction | AppAction)[]
        > = {
            app: appActions,
            turnInto: turnIntoActions,
        ...
        return (
            <CommandGroup key={group} heading={GROUP_LABELS[group]}>
                {items.map((item) => (
                    <CommandItem
                        key={item.id}
                        onSelect={() => runEditorAction(group, item)}
                    >
                        {item.swatch ? (
                            <ColorSwatch hex={item.swatch} />
                        ) : item.icon ? (
                            <item.icon />
                        ) : null}
    ...
    return (
        <CommandDialog open={open} onOpenChange={setOpen}>
            <Command shouldFilter={false}>
                <CommandInput
                    placeholder="Search a command or type to insert…"
                    value={query}
                    ...
                    {showRecents && (
                        <>
                            <CommandGroup heading="Recent">
                                {recents.map((entry) => (
                                    <CommandItem
                                        key={`recent-${entry.group}-${entry.id}`}
