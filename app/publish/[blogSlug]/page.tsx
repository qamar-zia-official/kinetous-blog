...
export default function Editor({
    ...
    // UI-level ownership gate. This is *not* the security boundary — that's
    // the requireOwner() check inside updateBlog/removeBlog, which runs
    // regardless of what this page does. This is purely so someone who opens
    // another author's post (e.g. by guessing/sharing a /publish/<slug> URL)
    // sees a clear "not yours" message and a disabled editor instead of
    ...
    // Runs once both the post and the session have loaded. If they mismatch,
    // lock the editor down.
    useEffect(() => {
        if (!session || !meta.values.id) return;
        setIsForeignPost(meta.values.authorId !== session.user.id);
    }, [session, meta.values.id, meta.values.authorId]);
    ...
    const canSave = isValid && !isSaving && !isForeignPost;
            ...
            {isForeignPost && (
                <div className="mx-auto max-w-6xl w-full px-6 pt-4">
                    <div className="rounded-lg border border-destructive/40 bg-destructive/10 px-4 py-3 text-sm text-destructive">
                        This post was written by someone else — you can read it
                        here but can&apos;t edit or save changes to it.
                    </div>
                </div>
            )}
                        ...
                        <div className="flex relative justify-center items-center w-fit mx-auto">
                            <CldUploadButton
                                ...
                                options={{
                                    ...
                                    publicId:
                                        JSON.parse(coverImageFile).public_id,
                                }}
                                onSuccess={(result) => {
                                    if (
                                        result.info &&
